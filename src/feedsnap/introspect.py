...
when_to_use: >
  Use feedsnap when you need to read RSS or Atom feeds and get structured output
  for further processing, display, or analysis. Supports single-feed and multi-feed
  (OPML) modes, date filtering, deduplication across runs, and ACLI-compliant JSON
  envelopes for agent pipelines.
version: {version}
acli_version: {acli_version}
---
...
Fetch an RSS or Atom feed and output a clean digest. This is the primary command.

```bash
# Basic markdown digest
feedsnap https://lobste.rs/rss

# Limit to 5 entries, ACLI JSON envelope
feedsnap https://lobste.rs/rss --limit 5 --output json

# Tabular output (TITLE | DATE | URL)
feedsnap https://lobste.rs/rss --output table

# Only entries from the last 2 days, skip already-seen entries
...
The `--output` flag controls how results are rendered. Use `--output json` for
ACLI-compliant structured output suitable for agent pipelines.

| Option | Type | Default | Description |
| ------ | ---- | ------- | ----------- |
| `--output` / `-o` | enum[text\\|json\\|table] | text | Output mode: `text` (markdown), `json` (ACLI envelope), `table` (aligned columns). |
| `--title` | bool | false | Include feed title as H1 header. Always on in OPML mode. |
| `--since DATE` | string | — | Only entries on or after this date. Accepts YYYY-MM-DD or Nd (e.g. `2d`). |
| `--dedup` | bool | false | Skip entries seen in a previous run (tracked in `~/.feedsnap/seen.db`). |
| `--seen-db PATH` | path | — | Custom SQLite DB for seen-entry tracking (implies `--dedup`). |
| `--limit` / `-n` | int | 8 | Max entries to return (per feed in OPML mode). |
| `--opml FILE` | path | — | OPML subscriptions file for multi-feed mode. |

### ACLI JSON envelope (`--output json`)

When `--output json` is used, the response follows the ACLI v0.1.0 envelope schema:
...
`--format markdown` and `--format json` still work for backward compatibility.
Prefer `--output text` and `--output json` in new scripts.

## Exit codes

| Code | Meaning |
| ---- | ------- |
| 0 | Success — feed fetched and output written to stdout. |
| 1 | Error — feed fetch failure, invalid arguments, or empty OPML. Error detail on stderr. |
...
- Run `feedsnap introspect` to get the full machine-readable command tree.
"""
...
def get_skill_md() -> str:
    ...
    Version fields are substituted at call time so the output always
